...
import { Population3DChart } from '@/components/features/chart/Population3DChart';
...
export default function ThreeDPage() {
  ...
  // 選択された都道府県の人口データを抽出
  const selectedPopulationData = new Map(
    Array.from(populationData || new Map()).filter(([prefCode]) => 
  ...
  return (
    <main className="container mx-auto px-4 py-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-8 mb-8">
        <h1 className="text-xl sm:text-2xl font-bold text-gray-900">都道府県別人口推移 3Dグラフ</h1>
      ...
      <section>
        ...
        {isLoadingPopulation ? (
          <ChartSkeleton className="w-full h-[500px]" />
        ) : populationError ? (
          <ChartErrorFallback />
        ) : selectedPopulationData.size > 0 ? (
          <Population3DChart
            prefectures={prefectures || []}
